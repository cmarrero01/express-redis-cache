module.exports = (function () {
  ...
  function del (name, callback) {
    ...
    /**  Delete cache entries with any prefix
     *
     *  @method ExpressRedisCache.delWithoutPrefix
     *  @description Delete entry by name without any prefix
     *  @return null
     *  @arg {String} name - The entry name
     *  @arg {Function} callback
     */

    function delWithoutPrefix (name, callback) {
        var self = this;
        ...
        domain.run(function delRun () {
            ...
            var hasWildcard = name.indexOf('*') >= 0;
            ...
            if ( hasWildcard ) {
                ...
                self.client.keys(name, domain.intercept(function onKeys (keys) {
            ...
            /** No wildcard **/
                ...
                self.client.delP(name,
                    domain.intercept(function onKeyDeleted (deletions) {
                        self.emit('message', require('util').format('DEL %s', name));
                        callback(null, +deletions);
                    }));
            }
